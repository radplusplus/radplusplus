...
frappe.views.CommunicationComposer = Class.extend({
	...
	prepare: function() {
		...
		// radpp - 2018-12-10 		
		this.dialog.fields_dict["standard_reply"].set_value(frappe.model.get_value("Print Format", this.dialog.fields_dict.select_print_format.get_value(), "standard_reply"));
		this.use_default_standard_reply	= true;
	...
	setup_standard_reply: function() {
		...
		this.dialog.fields_dict["standard_reply"].df.onchange = () => {
			...
			var prepend_reply = function(reply) {
				if(me.reply_added===standard_reply && !me.use_default_standard_reply) {
					return;
				}
				me.use_default_standard_reply = false;
				...
				// radpp - 2018-12-18				
				me.dialog.fields_dict["content"].set_value(content.join(''));	
				me.dialog.fields_dict["subject"].set_value(__(reply.subject,me.lang_code));
	...
	setup_print_language: function() {
		...
		this.lang_code = doc.language || frappe.boot.user.language;

		//On selection of language retrieve language code
		$(fields.language_sel.input).change(function(){
			me.lang_code = this.value
			// radpp - 2018-12-18
			me.use_default_standard_reply = true;			
			//me.dialog.fields_dict["content"].set_value("");	
			//me.dialog.fields_dict["subject"].set_value("");
			me.dialog.fields_dict["standard_reply"].df.onchange()
		})
	...
	setup_earlier_reply: function() {
		...
		if(this.real_name) {
			this.message = '<p>'+__('Dear',this.lang_code) +' '
